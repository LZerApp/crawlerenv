    ...
    def parse_product(self, item):
        ...
        try:
            ...
            try:
                ...
                sale_price = self.get_price(item.find("div",{"class":"Label-price sl-price is-sale primary-color-price"}).text)
        return Product(title, link, link_id, image_url, original_price, sale_price)
...
class MymybagCrawler(BaseCrawler):
    id = 381
    name = "mymybag"
    base_url = "https://www.mymybag.com.tw/SalePage/Index/"  # 要記得改
    query = """
    query cms_shopCategory($shopId: Int!, $categoryId: Int!, $startIndex: Int!, $fetchCount: Int!, $orderBy: String, $isShowCurator: Boolean, $locationId: Int) {
        shopCategory(shopId: $shopId, categoryId: $categoryId) {
            salePageList(startIndex: $startIndex, maxCount: $fetchCount, orderBy: $orderBy, isCuratorable: $isShowCurator, locationId: $locationId) {
...
"""
    ...
    variables = {"shopId": 40333, "categoryId": 369768, "fetchCount": 200, "isShowCurator": True, "locationId": 0}
...
class NjmusesCrawler(BaseCrawler):
    ...
    def parse_product(self, item):
        ...
        try:
            original_price = self.get_price(
                item.find("span", {"class": "price original-price currency"}).text)
            sale_price = self.get_price(
                item.find("span", {"class": "price currency"}).text)
        except:
            original_price = ""
            sale_price = self.get_price(
                item.find("span", {"class": "price currency"}).text)
...
class CharleneCrawler(BaseCrawler):
    id = 409
    name = "charlene"
    prefix_urls = ["https://www.charlene168.com/product-category/%e7%8f%be%e8%b2%a8%e5%95%86%e5%93%81/page/",
...
class BonnyreadCrawler(BaseCrawler):
    id = 162
    name = 'bonnyread'
    prefix_urls = ["https://www.bonnyread.com.tw/products?page={i}&limit=72"]
    urls = [f'{prefix}'.replace('{i}', str(i)) for prefix in prefix_urls for i in range(1, 10)]
...
class HealerCrawler(BaseCrawler):
    id = 190
    name = 'healer'
    prefix_urls = ["https://www.healer.com.tw/categories/%E6%89%80%E6%9C%89%E5%95%86%E5%93%81?page={i}"]
    urls = [f'{prefix}'.replace('{i}', str(i)) for prefix in prefix_urls for i in range(1, 50)]
    ...
    def parse_product(self, item):
        try:
            ...
            url = "https://www.healer.com.tw" + item.find('a').get('href')
            try:
...
class GirlsmondayCrawler(BaseCrawler):
    id = 301
    name = 'girlsmonday'
    url = 'https://www.girlsmonday.com.tw/PDList2.asp?item1=all&ob=A&pageno=&pagex=all'
    ...
    def parse(self):
        ...
        # time.sleep(10)
        soup = BeautifulSoup(response.text, features='html.parser')
        items = soup.find('div', {'class': 'pgNum'}).find_all('div', {'class': 'PDItem'})
        self.result.extend([self.parse_product(item) for item in items])

    def parse_product(self, item):
        try:
            url = 'https://www.girlsmonday.com.tw/' + item.find('a').get('href')

            prefix_product_id = parse.parse_qs(parse.urlparse(item.find('a').get('href')).query)

            product_id = prefix_product_id['yano'][0] + '-' + prefix_product_id['color'][0]
            title = item.find('div', {'class': 'PDItemName'}).text.strip()
            img_url = item.find('img')['src']
            try:
                ori_price = int(item.find('span', {'class': 's1 priceOriginal'}).text.split()[1])
            except:
                ori_price = ''
            sale_price = int(item.find('span', {'class': 's2 priceNow'}).text.split()[1])
        except:
            return

        return Product(title, url, product_id, img_url, ori_price, sale_price)
...
class TennyshopCrawler(BaseCrawler):
    id = 185
    name = 'tennyshop'
    prefix_urls = ['https://www.tennyshop.co/products?page={i}&limit=48']
    urls = [f'{prefix}'.replace('{i}', str(i)) for prefix in prefix_urls for i in range(1, 30)]
    ...
    def parse_product(self, item):
        try:
            ...
            title = (item.find('div', {'class': 'title'}).text).strip(' \n ')
            try:
                img_url = item.find('div', {'class': 'boxify-wrapper'}
                                    ).find('div').get('style').split('background-image:url(')[1].replace('?)', "")
            except:
                img_url = item.find('div', {'class': 'boxify-image js-boxify-image center-contain sl-lazy-image'}
                                    ).get('style').split('background-image:url(')[1].replace('?)', "")
...
class MypopcornCrawler(BaseCrawler):
    id = 186
    name = "mypopcorn"
    prefix_urls = ['https://www.mypopcorn.com.tw/categories/●所有商品?page={i}&limit=72']
...
class Red21Crawler(BaseCrawler):
    ...
    def parse_product(self, item):
        try:
            url=item.find('a').get('href')
            page_id=item.get('product-id')
            img_url=item.find('div',{'class':'boxify-image js-boxify-image center-contain sl-lazy-image second-image'}).get('style').split('(')[-1].replace('?)','')
            title=item.find('div',{'class':'title text-primary-color'}).text
            try:
                original_price=float(item.find('div',{'class':'global-primary dark-primary price sl-price price-crossed'}).text.strip(' \n ').replace("NT$","").replace(",",""))
            except:
                original_price=""
            try:
                sale_price=float(item.find('div',{'class':'global-primary dark-primary price sl-price'}).text.strip(' \n ').replace("NT$","").replace(",","")) 
            except:
                sale_price=item.find('div',{'class':'price-sale price sl-price primary-color-price'}).text.strip(' \n ').replace("NT$","").replace(",","").split("~")
                if(float(sale_price[0].strip())<float(sale_price[-1].strip())):
                    sale_price=float(sale_price[0].strip())
                elif(float(sale_price[0].strip())>float(sale_price[-1].strip())):
                    sale_price=float(sale_price[-1].strip())
    ...
    def parse_product(self, item):
        try:    
            ...
            try:
                ...
                page_id=url.split('&pid=')[-1]
            try:
                original_price=float(item.find('div',{'class':'prod-price'}).find('del').text.strip(' \n ').replace("NT.","").replace(",",""))
            except:
                original_price=""                
            try:
                sale_price=float(item.find('div',{'class':'prod-price'}).text.strip(' \n ').replace("NT.","").replace(",",""))        
            except:
                sale_price=float(item.find('span',{'class':'text-danger'}).text.strip(' \n ').replace("NT.","").replace(",",""))                
        except:
            return
        return Product(title, url, page_id, img_url, original_price, sale_price)
